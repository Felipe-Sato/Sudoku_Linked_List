...
int** finalCheck(int** mat){
	int i, j;
	while(verifyZeros(mat)){
		for(i = 0;i < 9; i++){
			for(j = 0;j < 9; j++){
				if(mat[i][j] == 0){
					List* linePos = checkLineColumn(mat, i, j);
					if(linePos != NULL)
						if(linePos->prox == NULL) mat[i][j] = linePos->info;
					else{
						List* quadPos = checkQuadrant(mat, i, j);
						if(quadPos != NULL)
							if(quadPos->prox == NULL) mat[i][j] = quadPos->info;
						else{
							List* commom = intersect(linePos, quadPos);
							if(commom != NULL)
								if(commom->prox == NULL) mat[i][j] = commom->info;
						}
					}
				}
			}
